...
class WikiScraper:
    ...
    def __init__(self, base_url, use_local_file=False, local_file_dir=None):
        """
        ...
        :param local_file_dir: Path to the HTML file
            (required if use_local_file is True).
        """
    ...
    def _get_soup(self, phrase):
        """
        Internal helper. Fetches HTML content
        ...
        if self.use_local_file:
            ...
            if (not self.local_file_dir or
                    not os.path.exists(self.local_file_dir)):
                # Fail strictly if the test directory itself is missing.
                raise FileNotFoundError(
                    f"Test directory not found: {self.local_file_dir}")

            formatted_phrase = phrase.replace(" ", "_")

            # Construct the filename: "tests/data/" + "Moon_Lord" + ".html".
            filename = f"{formatted_phrase}.html"
            file_path = os.path.join(self.local_file_dir, filename)

            if not os.path.exists(file_path):
                print(f"[!] Local test file not found: {file_path}")
                return None
            ...
            try:
                ...
                # Check status code, if <> 200 throw exception.
        ...
        # Parse raw text into BeauftifulSoup object.
    ...
    def get_table(self, phrase, table_number):
        ...
        # Some wikis use 'wikitable' class,
        # but finding 'table' tag makes sure no table is skipped.
        ...
        if index < 0 or index >= len(tables):
            print(f"[!] Table number {table_number}"
                  f" not found. (Found {len(tables)} tables).")
            return None
    ...
    def get_text(self, phrase):
        """
        Retrieves the full text of the article,
        excluding site navigation/menus.
        """
        soup = self._get_soup(phrase)
        if not soup:
            return None
        ...
        # For wiki.gg and MediaWiki, this is usually 'mw-parser-output'.
        content_div = soup.find('div', class_='mw-parser-output')
    ...
    def get_internal_links(self, phrase):
        """
        Finds valid internal links in the article to allow
        for recursive scraping. Returns a list of phrase
        strings (e.g., ['Eye_of_Cthulhu', 'King_Slime']).
        """
        soup = self._get_soup(phrase)
        if not soup:
            return []
        ...
        for a_tag in content_div.find_all('a', href=True):
            ...
            # Check for the /wiki/ prefix to ensure we stay on the site.
            # We also ignore special pages like:
            # 'File:', 'Special:', 'Talk:' to keep it relevant.
            if href.startswith('/wiki/') and ':' not in href:
                # Extract the phrase part: "/wiki/Moon_Lord" -> "Moon_Lord".
                clean_phrase = href.replace('/wiki/', '')
                clean_phrase = clean_phrase.split('#')[0]
